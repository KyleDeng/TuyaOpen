...
OPERATE_RET tuya_authorize_init(void)
{
    ...
    ret = tal_cli_cmd_register((cli_cmd_t *)&s_cli_cmd, sizeof(s_cli_cmd)/sizeof(cli_cmd_t));
...
static void cli_authorize_read(int argc, char *argv[])
{
    ...
    char uuid_buf[UUID_LENGTH + 1];
    char authkey_buf[AUTHKEY_LENGTH + 1];
    ...
    memcpy(uuid_buf, license.uuid, UUID_LENGTH);
